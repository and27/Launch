...
const getInitialStep = (questionnaire): string => {
  //estos nombres deberían ser los que correspondan con el nivel al que van cuando fallan una pregunta
  //ejemplo: si fallan la pregunta 2 van al nivel 0 step 3 (concept)
  ...
  const orderedQuestionnaire = questionnaire.sort((a, b) => b.id - a.id);

  orderedQuestionnaire.forEach(questionEntry => {
    if (questionEntry.answer === 'No') {
      initialStep = learningPathEntryLevels[questionEntry.id - 1];
    }
  });
  return initialStep;
};

export const generateLearningPath = questionnaire => {
  const initialStep = getInitialStep(questionnaire);
  const MAXSTEPS = 5;

  const initialStepIndex = completeLearningPath.findIndex(
    learningStep => learningStep.name === initialStep
  );

  const endStepIndex =
    initialStepIndex + MAXSTEPS > completeLearningPath.length
      ? completeLearningPath.length
      : initialStepIndex + MAXSTEPS;

  const learningPath = completeLearningPath.slice(
    initialStepIndex,
    endStepIndex
  );
